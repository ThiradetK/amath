        ...
        <div className="bg-amber-500/15 border border-amber-500/30 rounded-xl p-4 mb-5">
          {isTie ? (
            <>
              ...
              {isWinner && (
                <p className="text-emerald-400 font-bold text-sm mt-2">🎉 คุณเสมอ!</p>
              )}
            </>
          ) : (
            <>
              <p className="text-amber-700 text-sm mb-1">ผู้ชนะ</p>
              <p className="text-amber-300 font-black text-xl">{winner?.name}</p>
              <p className="text-5xl font-black text-white mt-1 tabular-nums">
                {winner?.score}
                <span className="text-lg text-white/40 ml-2">แต้ม</span>
              </p>
              {isWinner && (
                <p className="text-emerald-400 font-bold text-sm mt-2">🎉 คุณชนะ!</p>
              )}
            </>
          )}
        </div>
            ...
            return (
              ...
              >
                <span className="text-white/60">
                  {medals[i] ?? '  '} {p.name}{isMe ? ' (คุณ)' : ''}
                </span>
                <div className="flex items-center gap-2">
                  {/* Show rack penalty if available */}
                  {p.rackPenalty != null && p.rackPenalty > 0 && (
                    <span className="text-red-400/70 text-xs">-{p.rackPenalty}</span>
                  )}
                  {p.emptyHandBonus != null && p.emptyHandBonus > 0 && (
                    <span className="text-emerald-400/70 text-xs">+{p.emptyHandBonus}</span>
                  )}
                  <span className={`font-black tabular-nums ${isTopScore ? 'text-amber-300' : 'text-white'}`}>
                    {p.score}
                  </span>
                </div>
              </div>
