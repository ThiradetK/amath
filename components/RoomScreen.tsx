...
export default function RoomScreen({ send }: RoomScreenProps) {
  const { serverState, myPlayerId, isHost, goToLobby, message } = useGameStore();
    ...
    <div className="min-h-screen bg-[#0a0500] text-white flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-sm space-y-5">
        ...
        <div className="text-center">
          <p className="text-white/30 text-xs uppercase tracking-widest mb-1">รหัสห้อง</p>
          <div className="text-6xl font-black text-amber-400 tracking-widest" style={{ fontFamily: 'monospace' }}>
            {roomId}
          </div>
        ...
        <div className="bg-[#150900] border border-amber-800/30 rounded-2xl p-5">
          <h2 className="text-amber-400 font-bold text-sm uppercase tracking-widest mb-4">
            ผู้เล่น {connectedPlayers.length}/{serverState.players.length > 0 ? serverState.players.length : '?'}
          </h2>
            ...
            {serverState.players.map((p, i) => (
              <div key={p.id} className={`flex items-center gap-3 px-3 py-2.5 rounded-xl border
                ...
                <div className="flex-1">
                  <span className={`font-bold ${p.id === myPlayerId ? 'text-amber-300' : 'text-white/80'}`}>
                    {p.name}
                    {p.id === myPlayerId && <span className="text-amber-600 text-xs ml-1">(คุณ)</span>}
                  </span>
                </div>
                <div className={`w-2 h-2 rounded-full ${p.connected ? 'bg-emerald-400' : 'bg-red-500'}`} />
              </div>
            ))}
            {/* Empty slots */}
            {Array.from({ length: Math.max(0, 2 - serverState.players.length) }).map((_, i) => (
              <div key={`empty-${i}`} className="flex items-center gap-3 px-3 py-2.5 rounded-xl border border-dashed border-white/10 text-white/20">
        ...
        <div className="space-y-2">
          {isHost ? (
            <button onClick={() => send('START_GAME')} disabled={!canStart}
              ...
              {canStart ? '🚀 เริ่มเกม!' : `รอผู้เล่นเพิ่ม (${connectedPlayers.length}/2+)`}
            </button>
          ) : (
