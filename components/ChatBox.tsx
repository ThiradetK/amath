    ...
    <div className="flex flex-col h-full">
      <div className="flex-1 overflow-y-auto space-y-1 mb-2 min-h-0">
        {chatMessages.length === 0 && (
          <div className="text-white/20 text-[10px] lg:text-sm text-center py-3">ยังไม่มีข้อความ</div>
        )}
        {chatMessages.map((msg, i) => (
          <div key={i} className={`text-[10px] lg:text-sm px-2 py-1.5 rounded-lg ${msg.from === playerName ? 'bg-amber-900/30 text-amber-300 text-right' : 'bg-white/5 text-white/60'}`}>
            {msg.from !== playerName && <span className="font-bold text-white/40 block">{msg.from}</span>}
