    ...
    <div className="min-h-screen bg-[#0a0500] text-white flex flex-col">
      ...
      <div className="border-b border-amber-900/40 px-3 sm:px-5 py-3 flex items-center justify-between bg-[#120800]">
        <div className="flex items-center gap-2">
          <span>🔢</span>
          <span className="text-amber-400 font-black" style={{ fontFamily: 'Georgia, serif' }}>A-Math</span>
        </div>
      ...
      <div className="w-full max-w-lg mx-auto p-2 sm:p-5 space-y-4 sm:space-y-5">
        ...
        <div className="bg-[#150900] border border-amber-800/30 rounded-2xl p-3 sm:p-5">
          <h2 className="text-amber-400 font-black text-base sm:text-lg mb-3 sm:mb-4">➕ สร้างห้องใหม่</h2>
          {!creating ? (
            ...
            <div className="space-y-2 sm:space-y-3">
              <div>
                <label className="text-white/50 text-xs uppercase tracking-widest block mb-1 sm:mb-2">จำนวนผู้เล่น</label>
        ...
        {/* Join by code */}
        <div className="bg-[#150900] border border-amber-800/30 rounded-2xl p-3 sm:p-5">
          <h2 className="text-amber-400 font-black text-base sm:text-lg mb-2 sm:mb-3">🔑 เข้าห้องด้วยรหัส</h2>
          <div className="flex gap-1 sm:gap-2">
            <input
              value={joinId}
              onChange={e => setJoinId(e.target.value.toUpperCase())}
              onKeyDown={e => e.key === 'Enter' && joinId && handleJoin(joinId)}
            ...
            />
            <button onClick={() => joinId && handleJoin(joinId)} disabled={!joinId}
        ...
        <div>
          <div className="flex items-center justify-between mb-2 sm:mb-3">
            <h2 className="text-amber-400 font-black text-base sm:text-lg">🚪 ห้องที่เปิดอยู่</h2>
            <button onClick={() => send('GET_ROOMS')} className="text-white/30 hover:text-white/60 text-xs transition-all">🔄 รีเฟรช</button>
          </div>
          {roomList.length === 0 ? (
            ...
            <div className="space-y-1.5 sm:space-y-2">
              {roomList.map(room => (
                <div key={room.id} className="flex items-center justify-between bg-[#150900] border border-amber-800/25 rounded-xl px-2 sm:px-4 py-2 sm:py-3">
                  <div>
                    <span className="font-black text-amber-300 tracking-widest text-base sm:text-lg">{room.id}</span>
                    <span className="ml-2 sm:ml-3 text-white/40 text-xs sm:text-sm">{room.playerCount}/{room.maxPlayers} คน</span>
                  </div>
