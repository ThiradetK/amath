...
import { validatePlacement, validateEquation, getWordFromBoard } from "./gameLogic";
...
export const useGameStore = create<GameStore>((set, get) => {
  ...
  const validateAndSetInvalid = (placedThisTurn: { tile: Tile; row: number; col: number }[], localBoard: Cell[][] | null) => {
    if (!localBoard || placedThisTurn.length === 0) {
      return new Set<string>();
    }

    const isFirstMove = localBoard.every((row) =>
      row.every((cell) => !cell.tile || placedThisTurn.some((p) => p.row === cell.row && p.col === cell.col)),
    );
    
    // Use full validation to catch ALL errors
    const { valid } = validatePlacement(localBoard, placedThisTurn, isFirstMove);
    ...
    const direction = placedThisTurn.length > 1 && 
      [...new Set(placedThisTurn.map(p => p.row))].length === 1 ? "H" : "V";
    
    const mainEq = getWordFromBoard(localBoard, 
      placedThisTurn.map(p => ({ row: p.row, col: p.col })), 
      direction);
    
    if (mainEq.length > 0 && !validateEquation(mainEq)) {
      placedThisTurn.forEach((p) => {
        const onLine = direction === "H" ? p.row === placedThisTurn[0].row : p.col === placedThisTurn[0].col;
        if (onLine) invalidSet.add(`${p.row},${p.col}`);
      });
    }
    
    for (const p of placedThisTurn) {
      const crossEq = getWordFromBoard(localBoard, 
        [{ row: p.row, col: p.col }], 
        direction === "H" ? "V" : "H");
  ...
  handleServerMessage: (msg, send) => {
    ...
    switch (type) {
      ...
      case "STATE_UPDATE": {
        const state = msg.state as ServerState;
        const { placedThisTurn, myPlayerId: me, serverState: oldState } = get();
        ...
        const turnChanged = oldState?.currentPlayerIndex !== state.currentPlayerIndex;
          ...
          message:
            ...
            (get().serverState?.moveHistory.length || 0)
              ? (() => {
                  ...
                  if (last.type === "pass") return `${last.playerName} ผ่านตา`;
  ...
  commitMove: () => {
    ...
    const { valid, error } = validatePlacement(localBoard, placedThisTurn, isFirstMove);
